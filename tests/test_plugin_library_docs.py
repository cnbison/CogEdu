...
def test_docs_links_point_to_existing_files():
    ...
    Plugin Library docs 引用以下文件 (per §一~§八 + 相关文档):
      ...
      - cogedu/plugins/base.py
      - cogedu/plugins/registry.py
      - cogedu/plugins/first_party/
      - cogedu/persistence/plugin_registry_store.py
    ...
    # cogedu/plugins/first_party/ 目录存在
    first_party_dir = REPO_ROOT / "cogedu/plugins/first_party"
    assert first_party_dir.is_dir(), (
        f"Plugin Library docs references missing dir: cogedu/plugins/first_party/"
    )

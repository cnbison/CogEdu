...
**【2026-09-16 方向修正】**：基于内容层复盘（`docs/cogedu-复盘与方向修正-2026-09.md`）与维护者四项拍板，**Phase 4/5/6 全部顺延**，先行启动 **P0 内容验证冲刺 + UI 桌面优先重设计**（双轨并行，任务清单见**第 19 章**）。原第 16/17/18 章任务清单不变，仅时点后移（其中 17.2 节 5-1 试点 Capability 注册并入 P0-1a）。
...
### 10.1 UI 现代化（#9）细化落档（2026-09-14 勘察 → 2026-09-16 全量发布）
...
- **9-G 灰度验证与收官**：✅（2026-09-16 完成）真实进程灰度通过 + 维护者人工验收通过（学生端五 TAB / 讲解场景生成与复看 / 教师端 / 家长端 / 授权链路）。**双轨终点执行**：legacy 静态页删除（web/student 仅存 vanilla 三模块 + login.html + auth.js + vendor），约 20 条 legacy grep 锁删除或迁移至 React 源文件（锁语义不变），**克隆后须先 `cd web/frontend && npm run build`**。验收期累计发现并修复 9 个真缺陷（均带契约锁，清单见 CHANGELOG 2026-09-15/16 条目）。全量 1955 用例通过。
...
## 19. 方向修正落地（2026-09-16 拍板）：P0 内容验证冲刺 + UI 桌面优先重设计

### 19.0 背景与依据

- **复盘依据**：`docs/cogedu-复盘与方向修正-2026-09.md`（含同日 §7 拍板结论）。核心判断：内容层是空的（Q 矩阵 32 题全占位、`DEFAULT_CAPABILITIES_LIST` 为 Python 5 条、呈现引擎无内容校验），**系统从未用真实数理化内容跑过一次端到端**——继续向 Phase 4/5/6 堆工程能力是在未验证的地基上加高。
- **四项拍板（2026-09-16）**：①学科口径按学科起始年级（数学七年级/物理八年级/化学九年级，不绑死"初一"——标准课标初一无独立物理化学课）；②学生端设备以横屏平板/电脑为主；③UI 走桌面优先重设计（UI-R 独立立项）；④P0-1 先行、P0-2 紧随。
- **两处关键修正（代码级核查，详见复盘 §7.2）**：P0-2 闸门**不能直接复用** `dual_agent` 的 belief_check（校验对象是 BeliefState 不是生成内容），正确做法是借 HumanReviewTrigger 模式**新建内容级校验器**；内容工程除真题外还有**逐题 `partial_credit_rubric`**（LLM judge 4 档判分依赖，现为空）。

### 19.1 排期总图（双轨并行）

```
轨 1（内容轨，先行——不依赖 UI）               轨 2（UI 轨，可稍后启动）
P0-1a 三学科试点内容准备                        UI-R 桌面优先重设计（19.3）
（19.2）                                       信息架构设计稿（2026-09-16 五项决策全部拍板）→ 施工
              ↘                  ↙
      P0-1b 学生实测 + 人工评估（在新桌面 UI 上做，19.4）
                    ↓
      P0-2 呈现内容闸门（19.5，规则来自 P0-1b 发现的错误）
                    ↓
      三学科内容规模化（原 Phase 5 主体 5-2..5-6 + 教师协作填题工作流）
                    ↓
      Phase 4 证据链可视化（第 16 章，时点后移） → Phase 6（第 18 章）
```

原第 16/17/18 章任务清单内容不变，仅时点后移；其中 **17.2 节（5-1 试点 Capability/Goal 注册）并入 P0-1a**，不重复排期。

### 19.2 P0-1a：三学科试点内容准备（内容工程 + 管道贯通，不依赖 UI）
...
- [x] **UI-R-3 答题页双栏** ✅（2026-09-16 施工完成，待人工验收）：题目区（主，约 60%）+ 作答侧栏（约 40%）双栏并排（≥1024，桌面作答侧栏粘性跟随 position: sticky）；<1024 折叠为单栏上下排（侧栏带 margin-top: 16px）。题目区（.answer-main）含 answer-meta（Bloom/topic/探针/热身角标）/ prob / one-liner / LCA 系统决策折叠；作答侧栏（.answer-side）含 CodeEditor + self-conf-row 4档自评 + btns 提示/提交 + hint-box + 已答计数器（设计稿 §6 答题侧栏底部）。反馈框跨双栏：.feedback-box 提到 .answer-body 之外、.answer-page 根下，与设计稿 §6 主图"题目区+作答侧栏作为答题主体 + 反馈框作为下方全宽总结"一致。新增组件内 state `[answeredCount, setAnsweredCount]`：仅在 judged + persisted 成功路径 +1（AI 评判失败/持久化失败不计入）；题目切换不 reset，跨题目累加；不新增 API（跨 session 持久化留独立任务）。CSS 改动：删 `.answer-page { max-width: 720px; margin: 0 auto; }`（题目区被压根源）；新增 `.answer-body` 双栏 grid（grid-template-columns: minmax(0, 1.5fr) minmax(280px, 1fr); gap: 24px）+ @media (max-width: 1023px) 单栏折叠 + @media (min-width: 1024px) 粘性跟随。契约锁：新增 `TestAnswerPageDualColumnWiring` 5 例（.answer-page 无 720 约束 / .answer-body ≥1024 双栏 grid + sticky / <1024 单栏 / AnswerPage 双区结构 + answeredCount state / 提交成功 +1 判定）；全量 1977 用例通过（1972 → +5）。
- [x] **UI-R-4 四页面宽幅适配** ✅（2026-09-16 施工完成，待人工验收）：今天 / 我在哪 / 成长 / 报告 四页面在桌面（≥1024）利用宽幅；<1024 折叠为单栏，与移动端体验一致。① HomePage 3 卡 `.home-cards` auto-fit grid + MotivationPanel 下方单列（`home-layout` flex 容器承接）；② WherePage hero 全宽 + 4 section（5D / Bloom / TC / LearningDNA）整体包入 `.where-grid` 双栏 grid；③ GrowthPage 5D 折线图全宽 + 2 section（轨迹快照 + 答题历史）包入 `.growth-grid` 双栏 grid；④ ReportPage 删 JSX 内联 `style={{maxWidth: 720, margin: "0 auto"}}`，表格列宽走 `.report-page table { table-layout: auto; width: 100% }` 自适应（<1024 走 `overflow-x: auto` 横向滚动兜底）；打印走 `@media print { .report-page { max-width: none !important } }` 不受删改影响。契约锁：新增 `TestFourPagesDesktopLayoutWiring` 6 例（ReportPage 无 720 内联 + CSS 顶层无 max-width 锁 / HomePage .home-layout 容器 / WherePage .where-grid + 4 section 在 grid 内 / GrowthPage .growth-grid + 2 section 在 grid 内 / ≥1024 双栏 grid / <1024 单栏折叠媒体查询同时覆盖 .where-grid + .growth-grid）；全量 1983 用例通过（1977 → +6）。
- [ ] **UI-R-5 契约锁补齐 + 全量回归**（pytest + vitest + node:test）
- 设计稿侧栏移动降级（<768px 抽屉）已并入 UI-R-1 SidebarShell 落地，本节原"UI-R-3 移动降级"条目作废
- Phase 3 vanilla 模块挂载式整合（whiteboard/playback/formula.js 不动 + node:test 时序锁保留）由 UI-R-1 起即守住，本节不单列
- 教师端/家长端已是 1080px 顶栏形态，本项不动（后续视可视化需要再评估统一 Shell）

### 19.4 P0-1b：学生实测 + 人工评估（双轨汇合点）

- [ ] 在新桌面 UI 上做学生实测（不在旧 720px 形态上评估体验，避免"在错误形态上评估"）
- [ ] **三个同级评估对象**：① 讲解内容教学质量（正确性/讲解逻辑/年级认知水平适配）；② LLM judge 判分质量（数理化 partial credit——belief 更新全部建立在 judge 输出之上，垃圾进垃圾出）；③ Bloom 分层/误概念库/干预类型设计对三学科实际教学场景的适配度
- [ ] 评估产出的**错误类型清单** → P0-2 闸门规则的第一手输入；若发现 Bloom/误概念设计与实际教学场景对不上，回到 19.2 调整后再进入规模化

### 19.5 P0-2：呈现内容闸门

- [ ] 借 `anti_hallucination/human_review.py` 的 HumanReviewTrigger 模式（低置信 → 标记 → 复核队列），**新建内容级校验器**，挂 `presentation_service` 落库出口前（`on_scene` 回调点）：
  - **L1 程序化校验**（便宜、确定性）：LaTeX 语法合法性、数值重算一致性、物理单位量纲检查
  - **L2 LLM 交叉校验**（中等成本）：第二模型调用，专查公式推导与概念表述，输出置信度
  - **L3 复核**：低置信内容标记 `needs_review`，进教师复核队列（教师端已有路由/API 基础可复用）
- [ ] 优先覆盖高风险内容类型：数学公式推导、物理/化学概念表述
- [ ] **明确不做**：全量人工审核（成本不可承受）；直接调用 dual_agent `belief_check`（校验对象不同，见 19.0）

### 19.6 规模化阶段（P0 之后）

- **三学科内容规模化** = 原 Phase 5 主体（5-2 PDF 解析引擎 / 5-3 KnowledgeChunk / 5-4 检索接口 / 5-5 可信度分级复核——第 17 章任务清单不变）+ **教师协作填题工作流**（把"教师填 Q 矩阵"从一次性任务变成可持续流程，复盘 P1-4：先做最简表格协作 + 人工审核入库）
- **Phase 4 证据链可视化**（第 16 章不变）：有真实内容后再做，可视化"空内容"没有验证价值；现有教师端证据链视图（7 端点）已足以支撑 P0 阶段的人工评估，不需要提前做 4-C
- **Phase 6**（第 18 章不变）

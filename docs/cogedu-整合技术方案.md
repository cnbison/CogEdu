...
- [ ] 家长登录 + 绑定入口 + 授权管理页（发起绑定申请/查看已授权限/撤销）
- [ ] 学生端确认页（待确认的家长绑定申请，确认/拒绝）
- [ ] 学习总览仪表盘（复用学生端已有的 Belief 可视化组件，按家长视角做简化/汇总）
- [ ] 报告下载入口（周报/月报选择）
- [ ] （如果 v1 做证据链简化版）证据链展示页
- [ ] （如果 v1 做预警简化版）站内预警通知位（先做站内展示，不做推送渠道）

### 14.7 2-E：回归与发布

- [ ] 权限边界回归：学生撤销权限后家长端**下一次请求**立即失效、一个学生被多个家长/监护人关联互不干扰、家长尝试访问未获授权的学生数据被拒绝、pending 状态不可见任何数据
- [ ] 灰度：`scripts/canary_phase2_*.py`（延续 12.6/1-G 模式：真实进程 + 真实登录链路），重点人工复核**导出报告的可读性和实用性**（版式、公式是否清晰、数字家长能否看懂），先小范围家长用户验证，再全量
- [ ] 收尾：方案文档勾选 + CLAUDE.md/README/CHANGELOG 同步 + commit/push

...
10. **讲解生成等待体验**（2026-09-14 立项，Phase 3 验收暴露）：生成全程 10~25 分钟黑盒等待——串行逐场景生成 + 个别场景吃满超时重试，且全部场景生成完才一次性落库/返回。方向：① 逐场景落库 + 前端渐进渲染（生成一个显示一个）；② 场景生成结果服务端复用（同学生同知识点近期已生成则复看优先，配套已完成只读端点）；③ 重试/超时策略按耗时数据收紧。优先级：建议在 UI 现代化（#9）动工前先做①②（React 化会重写前端，先修静态页逻辑可平移）。

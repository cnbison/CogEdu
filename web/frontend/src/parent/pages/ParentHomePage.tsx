...
import GuardianLinksCard from "../components/GuardianLinksCard";
import { REPORT_PERIODS, downloadStudentReport, type ReportPeriod } from "../api";
...
export default function ParentHomePage() {
  ...
  if (!selectedId) {
    return (
      ...
      {/* CogEdu 2-A: 授权管理（家长侧）随列表视图展示 — 9-E */}
      <GuardianLinksCard />
    </>
  );
  }
  ...
  return (
    <div>
      ...
      <ReportDownloadCard studentId={data.student_id} />
...
// CogEdu 2-C: Word 学习报告下载（download_report 授权；403 = 无权/已撤销）
function ReportDownloadCard({ studentId }: { studentId: string }) {
  const [busy, setBusy] = useState(false);
  const [message, setMessage] = useState<string | null>(null);

  const download = async (period: ReportPeriod) => {
    setBusy(true);
    setMessage(null);
    try {
      await downloadStudentReport(studentId, period);
    } catch (e) {
    ...
    <div className="card" style={{ marginTop: 16 }}>
      ...
      <div style={{ display: "flex", gap: 10, marginTop: 10 }}>
        {REPORT_PERIODS.map((p) => (
          <button key={p} onClick={() => download(p)} disabled={busy}>
            {busy ? "生成中…" : p === "week" ? "本周" : p === "month" ? "本月" : "全部"}
          </button>
        ))}
      </div>
      {message && (
        <p className="muted" style={{ fontSize: 13, marginTop: 8 }}>{message}</p>
      )}
    </div>
  );
}

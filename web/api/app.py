...
# LLM 客户端（全局）
# 12.4 (0-C): get_llm 抽到 web/api/llm.py (框架无关), 此处 import 保持
# 本模块属性兼容 (过渡期), Flask 删除时 (12.4-6) 一并清理
from web.api.llm import get_llm  # noqa: E402
...
def api_get_version():
    ...
    try:
        import ecos
        return jsonify({"version": cogedu.__version__})
    except Exception as e:
...
# v0.56.1 / v0.58.0 / v0.99.0: judge 三件套抽到 web/api/judge.py (框架无关),
# 此处 import 保持本模块属性兼容 (过渡期), 12.4-6 删除 Flask 时一并清理
from web.api.judge import (  # noqa: E402
    _build_judge_prompt,
...
def api_get_report(student_id: str):
    ...
    v0.47.0 升级:
    - ecos_version 改为读 cogedu.__version__(避免硬编码漂移)

...
**方向修正（2026-09-16 拍板）**：内容层复盘（`docs/cogedu-复盘与方向修正-2026-09.md`）确认系统尚未用真实数理化内容跑过端到端（Q 矩阵全占位），**Phase 4/5/6 顺延**，先行双轨推进——**轨 1：P0-1a 三学科试点内容准备**（数学「一元一次方程」/物理「密度」/化学「物质的变化与性质」，真题 + 逐题评分 rubric + 误概念标注，原 Phase 5-1 并入）——**AI 起草版已完成（2026-09-16，`verified=false` 待教师复核，见 `docs/p0-1a-试点内容说明.md`）**；**轨 2：UI-R 学生端桌面优先重设计**（学生设备口径定为横屏平板/电脑为主，工作台形态替代底部 Tab，白板讲解全幅呈现）——**UI-R-1 SidebarShell 工作台已完成（2026-09-16，维护者初验通过）**：三形态左侧栏（≥1024 展开 220px / 768-1023 收窄 64px 图标栏 / <768 抽屉）替代旧底部 5-Tab；8 个现有路由全部升一级导航 + Phase 4-6 槽位占位；Phase 3 vanilla 白板/播放模块零改动；契约锁 `TestSidebarShellWiring` 8 例；全量 1963 用例通过。**UI-R-2 讲解场景页桌面布局已完成（2026-09-16，待人工验收）**：白板破 720 版心（16:9 等比缩放至全幅），字幕/大纲双栏（≥1024）/ 单栏 + 大纲抽屉 toggle（768-1023）/ 退化形态（<768）；ScenePlayer 重构为瘦宿主（暴露 onSubtitleChange + renderControls 控制回调）；ScenePage 控制条合并为单行（prev/play/replay/next）；新增 OutlinePanel 大纲面板；scene.css 桌面布局重写（破 .wrap + 双栏 grid + 大纲抽屉 + vanilla 契约 CSS 保留）；契约锁 `TestScenePageDesktopWiring` 9 例；全量 1972 用例通过。**UI-R-3 答题页双栏已完成（2026-09-16，待人工验收）**：题目区（主，约 60%）+ 作答侧栏（约 40%）双栏并排（≥1024 桌面侧栏粘性跟随）；<1024 折叠为单栏上下排。题目区含 answer-meta / prob / one-liner / LCA 系统决策折叠；作答侧栏含 CodeEditor / 4档自评 / 提示+提交按钮 / 提示框 / 本场已答 N 题计数器；反馈框跨双栏；新增组件内 answeredCount state（仅 judged + persisted 成功路径 +1）；`tests/test_frontend_react_wiring.py` 新增 `TestAnswerPageDualColumnWiring` 5 例；全量 1977 用例通过。**UI-R-4 四页面宽幅适配已完成（2026-09-16，待人工验收）**：今天（3 卡 auto-fit + MotivationPanel 下方）；我在哪（hero 全宽 + (5D+Bloom) / (TC+LearningDNA) 双栏）；成长（5D 折线图全宽 + 轨迹快照 / 答题历史 双栏）；报告（删 720 内联锁，表格列宽自适应，打印仍走 `@media print` 全宽）。新增 `TestFourPagesDesktopLayoutWiring` 6 例；全量 1983 用例通过。下一步：UI-R-5 契约锁补齐 + 全量回归（设计稿 §11）。任务清单见[整合技术方案第 19 章](docs/cogedu-整合技术方案.md)。

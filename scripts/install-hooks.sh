#!/usr/bin/env bash
# CogEdu: 一键安装 git hooks (模式继承自 ECOS v0.64.1)
#
# 用法: bash scripts/install-hooks.sh
#
# 作用: 把 githooks/ 目录设为本地仓库的 hooks 源.
#       之后所有 git commit 都会自动跑零 mutation AST 扫描 (pre-commit),
#       git push 额外跑全量 pytest (pre-push).
#
# 为什么需要这个:
...
echo ""
echo "  接下来:"
echo "    - git commit  → 自动跑零 mutation AST 扫描 (~1s)"
echo "    - git push    → 自动跑同一扫描 + pytest 全量 (~25s)"
echo ""
echo "  紧急绕过 (不推荐):"
